  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans">
      ...
      <div className="p-8 md:p-16">
        <h2 className="text-4xl font-semibold mb-4 text-gray-800">{recommendProducts.length} loại thuốc đã được đề xuất</h2>
        <p className="text-gray-600 text-lg mb-8">
          Những yếu tố thiết yếu cho thói quen chăm sóc sức khỏe hàng ngày của bạn. Chúng nhắm đến những nhu cầu ưu tiên của bạn.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {recommendProducts.map((product, index) => (
            <div key={index} className="bg-white rounded-lg shadow-sm p-4 flex flex-col items-start text-left border border-gray-100 relative">
              ...
              <img src={product.image} alt={product.name} className="h-48 object-cover mx-auto mb-4" />

              <h3 className="text-lg font-semibold text-gray-800 mb-1">{product.name}</h3>
              <p className="text-gray-500 text-sm leading-tight mb-4 line-clamp-2">{product.description}</p>
